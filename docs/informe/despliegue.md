...
- [2. Publicación de las imágenes](#publicacion-de-las-imagenes)
...
Este comando construye la imagen de MkDocs, descarga la imagen pública de Azure Vote Front y publica ambas imágenes en el ACR desde la VM.
...
    ```sh
    ...
    ssh -i ~/.ssh/id_rsa darioreyesr25@{{ terraform_output("vm_public_ip") }}
...
2. Ejecutar Ansible apuntando a la VM. Asegurarse de que el comando se ejecuta desde `./ansible`. Para forzar a Ansible a recrear todo desde el principio, es posible usar los argumentos `--force-handlers` y `--extra-vars "recreate=true"`.
...
    ??? Noté "Mostrar contraseñas guardadas en el vault".
...
El despliegue de la aplicación en el clúster de Kubernetes se realiza mediante Ansible, aplicando los manifiestos necesarios para crear el namespace, el deployment, el PersistentVolumeClaim, el Service y el secret de acceso al ACR. Todo el proceso queda automatizado en el playbook.




>  Importante: Este flujo está pensado para ejecutarse desde WSL/Ubuntu (o cualquier Linux con Ansible instalado). En Windows no está disponible `ansible-playbook` ni `kubectl` por defecto.
...
    Si muestra `<pending>`, espera unos minutos y vuelve a ejecutar el comando hasta que aparezca la IP.
...
A continuación se documentan los problemas más relevantes surgidos durante la ejecución y su solución.




### WSL montado en modo solo lectura

- Qué pasaba: Al ejecutar o instalar paquetes, WSL mostraba errores como "mounted read-only as a fallback".

- Solución: Reiniciar WSL (`wsl --shutdown`) y corregir el fichero `~/.wslconfig` para que tenga la sección `[wsl2]` bien formada. Luego volver a abrir el shell y ejecutar `sudo apt update`.

- Referencias: Microsoft Docs (2024). Recuperación de WSL en modo solo lectura. https://aka.ms/wsldiskmountrecovery




### Plugin faltante en MkDocs

- Qué pasaba: `mkdocs build`Fallaba porque no existía''.

- Solución: Instalar dependencias desde dentro de WSL:
...
### Autenticación ACR desde AKS (AcrPull)

- Qué pasaba: El clúster no podía descargar las imágenes del ACR porque no tenía permiso.

- Solución: Terraform creó un `azurerm_role_assignment` con `role_definition_name = "AcrPull"` sobre la identidad gestionada del AKS, y Ansible creó el secret `acr-secret` en Kubernetes.




### Advertencias de MkDocs/Material 2.0

- Qué pasaba: MkDocs mostraba advertencias sobre MkDocs 2.0 y que los plugins podrían dejar de funcionar.

- Solución: Se mantiene la versión 1.x en `requirements.txt` para evitar romper el build.
